IRC is used by Electrum Twist server to find 'peers' - other Electrum servers. The
current list can be seen by running:

  electrum-twist-server peers
...
mode, `electrum-twist-server peers` will always return an empty list.
...
composing the IRC nickname to identify your server on #electrum-twist.
